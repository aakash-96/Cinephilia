...
public class PnlDescription extends JPanel{
	JLabel lDescription;
	JTextField tDescription;
	DefaultTableModel mMovies;
	JTable tbMovies;
	PnlDescription(final FrmSearch a){
		setLayout(null);
		lDescription=new JLabel("Description");
		lDescription.setBounds(475,40,130,60);
		lDescription.setFont(new Font("Papyrus",Font.BOLD,18));
		add(lDescription);
		
		tDescription=new JTextField();
		tDescription.setBounds(625,55,170,30);
		add(tDescription);
	...
	void fillMovies(String name){
		try{
			...
			PreparedStatement ps=con.prepareStatement("Select * From Movies where description like'%"+name+"%'");
	...
	void Listeners(){
		tDescription.addKeyListener(new KeyListener(){
			public void keyPressed(KeyEvent ke){
			}
			public void keyReleased(KeyEvent ke){
				
				RemoveMovies();
				fillMovies(tDescription.getText());
			}
