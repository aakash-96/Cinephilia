...
public class PnlDirector extends JPanel{
	JLabel lDirector;
	JTextField tDirector;
	DefaultTableModel mMovies;
	JTable tbMovies;
	
	PnlDirector(final FrmSearch a){
		setLayout(null);
		lDirector=new JLabel("Enter the Name of Director");
		lDirector.setBounds(370,40,220,60);
		lDirector.setFont(new Font("Papyrus",Font.BOLD,16));
		add(lDirector);
		
		tDirector=new JTextField();
		tDirector.setBounds(640,55,170,30);
		add(tDirector);
	...
	void fillMovies(String name){
		try{
			...
			PreparedStatement ps=con.prepareStatement("Select * From Movies where DirectedBy like'%"+name+"%'");
	...
	void Listeners(){
		tDirector.addKeyListener(new KeyListener(){
			public void keyPressed(KeyEvent ke){
			}
			public void keyReleased(KeyEvent ke){
				
				RemoveMovies();
				fillMovies(tDirector.getText());
			}
